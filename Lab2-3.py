...
if __name__ == "__main__":
    ...
    for ip, times in per_ip_timestamps.items(): #Ip address is printed and timestamps for each entry for that IP are displated
        print(ip)
        for x in times:
            print("\t",x)
    incidents = []
    ...
    for ip, times in per_ip_timestamps.items():
        ...
        while i < n:
            ...
            if count >= 5:
                ...
                i += 1
    print("Number of detected incidents",len(incidents))
    print(incidents[0])
    print(incidents[1])
    print(incidents[2])
    print(incidents[3])
